"""
 This file is copied from https://github.com/facebookresearch/demucs/blob/main/demucs/solver.py
...
from demucs import augment, states, pretrained
...
from train_env.htdemucs_musdbhq import distrib
...
class Solver(object):
    def __init__(self, loaders, model, fmodel, optimizer, args):
        self.args = args
    ...
    def _run_one_epoch(self, epoch, train=True):
        ...
        for idx, sources in enumerate(logprog):
            ...
            if not train and self.args.valid_apply:
                ...
                estimate = torch.nan_to_num(estimate)
            else:
            ...
            if args.optim.loss == 'l1':
                if train and args.kd_lambda>0:
                    sdrs, sdrqs = [], []
                    with torch.no_grad():
                        festimate = self.fmodel(mix).detach()
                        for i in range(len(festimate)):
                            sdr = new_sdr(sources[i:i + 1], festimate[i:i + 1]).detach()
                            sdrs.append(sdr)
                            sdrq = new_sdr(sources[i:i + 1], estimate[i:i + 1]).detach()
                            sdrqs.append(sdrq)
                        sdrs = torch.concatenate(sdrs, dim=0)
                        sdrqs = torch.concatenate(sdrqs, dim=0)
                        w = torch.exp((sdrs - sdrqs) / 10)

                    # Task loss
                    task_loss = F.l1_loss(estimate, sources, reduction='none')
                    task_loss = task_loss.mean(dims).mean(0)

                    # KD loss
                    kd_losses = []
                    for i in range(len(estimate)):
                        kd_loss_i = F.l1_loss(estimate[i:i + 1], festimate[i:i + 1], reduction='none')
                        kd_losses.append(kd_loss_i)
                    kd_loss = torch.concatenate(kd_losses, dim=0)
                    kd_loss = kd_loss.mean(dims)
                    kd_loss = torch.mean(w*kd_loss, dim=0)
                    loss = (1-args.kd_lambda)*task_loss + args.kd_lambda*kd_loss
                else:
