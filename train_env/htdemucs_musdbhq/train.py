"""
 This file is copied from https://github.com/facebookresearch/demucs/blob/main/demucs/train.py
...
import os
import sys
from dora import hydra_main  # packages: hydra-core, hydra-colorlog
import hydra
...
from demucs.states import capture_init, load_model
...
from train_env.htdemucs_musdbhq.solver import Solver
...
def get_pretrained_model(url: str, model_name):
...
def get_solver(args, model_only=False):
    ...
    # Load model
    model, fmodel = create_pretrained_model(args.model_cfg, args.use_pretrained_weights)
    if args.kd_lambda > 0:
    ...
    # Construct Solver
    return Solver(loaders, model, fmodel, optimizer, args)


@hydra_main(config_path="../../configs", config_name="htdemucs", version_base="1.1")
def main(args):
    global __file__
    ...
    logger.info(args.model_cfg)
    logger.info(args.dset)
    logger.info(args.optim)
    logger.info(args.testing_cfg)

    # -------------------------------
    # Model
    # -------------------------------
    args.model_cfg.update({"model_path":args.pretrained})
