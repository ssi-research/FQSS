...
class ConvTasNetQ(nn.Module):
    ...
    *Conv-TasNet: Surpassing Ideal Time–Frequency Magnitude Masking for Speech Separation*
    ...
    def quantize_model(self, gradient_based=True,
        ...
        params_dict = {'gradient_based': gradient_based, 'act_quant': act_quant,
                       'weight_quant': weight_quant, 'weight_n_bits': weight_n_bits, 'act_n_bits': act_n_bits}
        ...
        for n, m in self.named_modules():
            if type(m) == ConvTasNetQ:
                ...
                quantize_modules(m.shared_block, ['0', '1'], params_dict)
                quantize_modules(m.shared_block, ['2'], params_dict)
                quantize_modules(m.shared_block, ['3', '4'], params_dict)
                quantize_modules(m.shared_block, ['5'], params_dict)
                quantize_modules(m, ['res_conv'], params_dict)
                quantize_modules(m, ['skip_conv'], params_dict)
                quantize_modules(m, ['add'], params_dict)
            elif type(m) == MaskGenerator:
                quantize_modules(m.bottleneck, ['0'], params_dict)
                quantize_modules(m.bottleneck, ['1'], params_dict)
                quantize_modules(m.mask_net, ['0'], params_dict)
                quantize_modules(m.mask_net, ['1', '2'], params_dict)
