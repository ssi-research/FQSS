...
from quantization.qat.models.demucsq import DConv, rescale_module
...
class ScaledEmbedding(nn.Module):
    ...
    def forward(self, x):
        out = self.mul(self.embedding(x), self.scale)
        return out
...
class HEncLayer(nn.Module):
    ...
    def forward(self, x, inject=None):
        ...
        if self.rewrite:
            z = self.glu(self.norm2(self.rewrite(y)))
        else:
            z = y
        return z
...
class HDemucsQ(nn.Module):
    ...
    def pre_process(self, mix):
        ...
        # Pre-process
        # ----------
        x = preprocess(x, n_splitter=self.n_splitter, threshold=85)
        if self.hybrid:
            xt = preprocess(xt, n_splitter=self.n_splitter, threshold=5)
            return x, xt, z

        return x, None, z

    def post_process(self, x, xt, z):
        ...
        for n, m in self.named_modules():
            if type(m) == HEncLayer:
                if n in ['encoder.0','tencoder.0']:
                    ...
                    if m.empty:
                        quantize_modules(m, ['conv'], params_dict)
                    else:
                ...
                for layer in m.layers:
                    if m.use_lstm:
                        ...
                        quantize_modules(layer[3], ['lstm'], params_dict)
                        quantize_modules(layer[3], ['linear'], params_dict)
                        quantize_modules(layer, ['5','6','7'], params_dict)
                        quantize_modules(layer[8], ['mul'], params_dict)
                    else:
                        quantize_modules(layer, ['0','1','2'], params_dict)
